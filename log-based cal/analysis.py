...
def findTodayRecord(name, dateName, fileSuffix):
    x = []
    with open(unicode('out/'+ dateName + fileSuffix, 'utf-8'), 'r') as inFile:
        for i in inFile:
            if name in i:
...
def importLog(name, dateName):
    ...
    y = []
    yName = []
    with open(unicode('analyse/汇总表-'+ dateName + '.txt', 'utf-8'), 'r') as inFile:
        for i in inFile:
            if name in i:
                numRecord = numFormat(i)
...
def analyse(buyer, dateList):
    a = findTodayRecord(buyer, dateList[-1], '-券商.txt')
    b = findTodayRecord(buyer, dateList[-1], '-券商三日.txt')
